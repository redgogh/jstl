package com.redgogh.common.collection;
 ...
 *     <li>集成了 {@link StreamMapper} 接口，允许通过 Lambda 表达式对集合元素进行
 *         自定义处理。</li>
 * </ul>
 *
 ...
 *     <li>对于需要映射转换的操作，需要传入实现了 {@link StreamMapper} 的 Lambda 表达式
 *         或函数式接口。</li>
 * </ul>
 *
...
public class Lists {
    ...
    /**
     * 通过实现 {@link StreamMapper} 的 Lambda 接口，将一个对象转换成另一个对象实例，并
     * 批量添加到新的 {@link List} 集合中。
     *
     * @param builder
     *        Lambda 函数实现接口，或者也可以通过实现 {@link StreamMapper} 接口的方式
     *        完成这个参数。
     *
     * @param a
     *        输入数组
     *
     * @return 返回通过 {@link StreamMapper} 转换后的集合实例。
     */
    public static <T, R> List<R> map(T[] a, StreamMapper<T, R> builder) {
     ...
     * @return 返回通过 {@link StreamMapper} 转换后的集合实例。
     */
    public static <T, R> List<R> map(Collection<T> collection, StreamMapper<T, R> builder) {
        List<R> retval = null;
        if (collection != null) {
            retval = of();
     ...
     * @return 计算后返回：两个集合之间的交集
     */
    public static <E, M> List<E> intersection(Collection<E> a, Collection<M> b, StreamMapper<M, E> bMapper) {
     ...
     */
    public static <M1, M2, E> List<E> intersection(Collection<M1> a, StreamMapper<M1, E> aMapper,
                                              Collection<M2> b, StreamMapper<M2, E> bMapper) {
     ...
     * @return 计算后返回：两个集合之间的差集
     */
    public static <E, M> List<E> diff(Collection<E> a, Collection<M> b, StreamMapper<M, E> bMapper) {
     ...
     * @return 计算后返回：两个集合之间的差集
     */
    public static <M1, M2, E> List<E> diff(Collection<M1> a, StreamMapper<M1, E> aMapper,
                                               Collection<M2> b, StreamMapper<M2, E> bMapper) {
     ...
     * @return 计算后返回：两个集合之间的差集
     */
    public static <E, M> List<E> symdiff(Collection<E> a, Collection<M> b, StreamMapper<M, E> bMapper) {
     ...
     * @return 计算后返回：两个集合之间的差集
     */
    public static <M1, M2, E> List<E> symdiff(Collection<M1> a, StreamMapper<M1, E> aMapper,
                                                   Collection<M2> b, StreamMapper<M2, E> bMapper) {
